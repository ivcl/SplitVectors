...
vector<double> CIEColor::getSpiralColor(double value, double H1, double H2, double contrast)
{
	if( H1>H2 )
		H2 = H2+360;
...
vector<double> CIEColor::getSpiralTrace(double value, double H1, double H2)
{
	...
	double r, g, b;	
	//closestLCHuvtoRGB(value*100, 200, H, r, g, b);
	vector<double> LCH = CIEColor::SinglehueSequential_LCH(value, 0.6, 0.25, 0.75, H);
	//vector<double> RGB;	RGB.push_back(r); RGB.push_back(g); RGB.push_back(b);
	return LCH;
}
