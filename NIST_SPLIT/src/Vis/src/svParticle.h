...
class svParticle {
 public:
  ...
  virtual void Advect(svInt direction, svScalar advection_time);
  virtual void Step(svInt direction);
  virtual void Step(svInt direction, svScalar max_advection_time);

 public:
  svVector3 position;       // the current particle position
  svScalar  time;           // the current time stamp
 
  // the vector field associated with the particel
  svVectorField * field; 
};
