...
vector<double> CIEColor::DiverginghueSequential(double t, double s, double b, double c, double h1, double h2, double w, double m)
{
	double h;
	double tprime;
	double e = m - 0.5;
	...
	{
		tprime = (t+fabs(2*e))/(1 + fabs(2*e));
	}

	
	if(fabs(tprime-0.5)<=0.0001)
	{
		tprime = 0.5;
		...
		c0 = SinglehueSequential_LCH(1,s,b,c,h1);//cerr<<c0[0]<<endl;
		c1 = SinglehueSequential_LCH(1,s,b,c,h2);//cerr<<c1[0]<<endl;
		cn[0] = (c0[0]+c1[0])/2;
		...
		pbright[0] = luv[0];
		LuvtoLCHuv(luv[0], luv[1], luv[2], pbright[1], pbright[2]);
		double tmp = pbright[2];
		cn[2] = tmp;

		double sn;
		sn = (c0[1]+c1[1])/2. * w;
		//smax(cn.L, cn.H)
		vector<double> pmid = dividingLCHuvforMSC_H(0,100,cn[2]);
		vector<double> pend(3,0);
		if(cn[0]<=pmid[0])
		{
			pend[0] = 0;pend[1] = 0;pend[2] = 0;
		}
		else
		{
			pend[0] = 100;pend[1] = 0;pend[2] = 0;
		}
		double alpha = (pend[0] - cn[0]) / (pend[0] - pmid[0]);
		double Sm = alpha * (pmid[1] - pend[1]) + pend[1];
		...
		//cerr<<cn[0]<<" "<<cn[1]<<" "<<cn[2]<<endl;

		double r, g, k;
		closestLCHuvtoRGB(cn[0], cn[1], cn[2], r, g, k);
	...
	else if(tprime<0.5)
	{
		h = h1;
		t = 2 * tprime;//cerr<<t<<" ";
		Pseg = SinglehueSequential(t,s,b,c,h1);	
	}
	else if(tprime>0.5)
	{
		h = h2;
		t = 2*(1-tprime);//cerr<<t<<" ";
		Pseg = SinglehueSequential(t,s,b,c,h2);		
	}
